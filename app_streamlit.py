...
YEARS = list(range(2024, 2017, -1))  # 2025+ non disponibles sur FastF1
...
def make_delta_figure(data1: dict, data2: dict) -> go.Figure:
    ...
    fig.update_layout(
        title=dict(
            text=f"DELTA VITESSE — {data1['driver']} vs {data2['driver']}",
...
def make_track_map(data1: dict, data2: dict) -> go.Figure:
    """
    Carte du circuit colorée selon la vitesse du pilote 1,
    avec superposition de la trajectoire du pilote 2.
    """
    t1 = data1["telemetry"]
    t2 = data2["telemetry"]
    c2 = data2["color"] or DRIVER_COLORS["default_2"]
    ...
    # Trajectoire pilote 1 colorée par vitesse
    fig.add_trace(go.Scatter(
        x=t1["X"], y=t1["Y"],
        mode="markers",
        marker=dict(
            ...
            size=3,
            colorbar=dict(
                title=dict(text="km/h", font=dict(color="#888", size=10)),
        ...
        name=f"{data1['driver']} ({data1['year']})",
        hovertemplate="Vitesse: %{marker.color:.0f} km/h<extra></extra>",
    ))

    # Trajectoire pilote 2 en ligne
    fig.add_trace(go.Scatter(
        x=t2["X"], y=t2["Y"],
        mode="lines",
        line=dict(color=c2, width=1.5, dash="dot"),
        opacity=0.6,
        name=f"{data2['driver']} ({data2['year']})",
        hoverinfo="skip",
    ))
    ...
    fig.update_layout(
        ...
        title=dict(
            text="CARTE DU CIRCUIT — Vitesse (pilote 1)",
            font=dict(family="Barlow Condensed", size=13, color="#e8002d"),
        ),
    )
    return fig
...
with col1:
    ...
    <div class="driver-card" style="--color: {c1_hex}">
        <div class="driver-name">{driver1}</div>
...
with col2:
    ...
    <div class="driver-card" style="--color: {c2_hex}">
        <div class="driver-name">{driver2}</div>
...
if "X" in t1.columns and "Y" in t1.columns:
    st.markdown('<div class="section-title">Carte du circuit</div>', unsafe_allow_html=True)
    fig_map = make_track_map(data1, data2)
    if fig_map:
