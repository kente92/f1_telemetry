...
def make_track_delta_map(data1: dict, data2: dict, label1: str, label2: str) -> go.Figure | None:
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(
            color=delta,
            colorscale=[
                [-1, None],   # sera ignoré, on définit ci-dessous
                [0.0,  "#e8002d"],   # rouge = p2 plus rapide
                [0.5,  "#222222"],   # neutre
                [1.0,  "#00d2ff"],   # bleu = p1 plus rapide
            ],
            colorscale=[
                [0.0, "#e8002d"],
                [0.5, "#1e1e2e"],
    ...
    title = f"DELTA CARTE — {label1} <span style='color:#00d2ff'>bleu</span> / {label2} <span style='color:#e8002d'>rouge</span>"
